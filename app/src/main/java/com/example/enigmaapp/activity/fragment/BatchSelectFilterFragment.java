...
public class BatchSelectFilterFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        switch (mFilterType) {
            ...
            case "counterparty":
                final CounterpartyFilterAdapter counterpartyAdapter = new CounterpartyFilterAdapter(requireActivity());
                ...
                counterpartyAdapter.setOnItemClickListener(new CounterpartyFilterAdapter.OnItemClickListener() {
                    @Override
                    public void onItemClick(TradeDatasetCounterparty counterpartiesItem, int position) {
                        System.out.println(" Clicked : " + counterpartiesItem.getName());

                        if (counterpartiesItem.getIsChecked()) {
                            counterpartiesItem.setIsChecked(false);
                            Iterator it = params.entrySet().iterator();
                            while (it.hasNext()) {
                                Map.Entry entry = (Map.Entry) it.next();
                                if (entry.getKey().equals("counterparty_id") && counterpartiesItem.getId().equals(entry.getValue())) {
                                    it.remove();
                                }
                            }
                        } else {
                            counterpartyAdapter.setLastCheckedPos(position);
                            counterpartiesItem.setIsChecked(true);
                            lastBatchCounterpartyPos = position;
                            params.put("counterparty_id", counterpartiesItem.getId());
                            prefEditor.putString("counterpartyBatchFilter", counterpartiesItem.getName());
                            prefEditor.apply();
                        }
    ...
    private void resetParam() {
        switch (mFilterType) {
            ...
            case "counterparty":
                ...
                viewModel.removeFromParams("counterparty_id");
                break;
            default:
                break;
        }
